...
const passport = require('passport')  // It is authentication middleware for node js
const flash = require('express-flash')  //It is use to define message and render  it without redirecting the request. 
const session = require('express-session') // It is a middleware use to create session. 
const bcrypt = require('bcryptjs') //It is module use to hass the password. 
const cookieParser = require('cookie-parser'); //It is use to parse the cookie.
const auth = require('../setting/auth-middleware')
const verifyPin = require('../setting/verifyPin')


const sgMail = require('@sendgrid/mail')
sgMail.setApiKey(process.env.SEND_GRID_KEY)   // This function is use to set sendGrid key. 
...
router.use(passport.initialize()) // It just initialize the passport module.
router.use(passport.session())  // this function update (or can say add) the user value in req (using req.user we can get username)
router.use(flash())
...
router.get('/eventDashboard', async (req, res) => {
  ...
  if (req.query.eventFlag == 'eventDashboard') {
    ...
    const EventDates = []   // It array is use store 'event' on particular 'date' and also  registered 'users' for that event (see event dashboard in UI)

   
    var randomisedList = await randomisedUser() // this will call to randomisedUser function, which will arrange all users randomly and return.
    ...
    for (var i = 0; i < dateEvents.length; i++) {
      await dateEvents[i].populate({    // this will compaire the date from dateEvent model and indoorEvent model, and retrive data from indoorEvent model.
        path: 'indoorEventDate'
      }).execPopulate()
      await dateEvents[i].populate({   // this will compaire the date from dateEvent model and outdoorEvent model, and retrive data from outdoorEvent model.
        path: 'outdoorEventDate'
      }).execPopulate()

      if (dateEvents[i].indoorEventDate.length !== 0 && dateEvents[i].outdoorEventDate.length !== 0) { // this check wether both indoor and outer is having same date or not.
        let eventArray = [] // this array is used to store only event of particular date.
        let eventObject = []
  ...
  // Above is use to create one single array which should contain event on particular date and also user register for that event (see event dashboard in UI)
  /********* value of EventDates 
   * [
        [
          [ 'cricket', [Array] ],
...
async function randomisedUser() {
  ...
  for (var i = 0; i < indoorEvents.length; i++) {
    await indoorEvents[i].populate({  // here we will compare indoorEvent from indoorEvent model and eventRegistered model and data from event registermodel is retrived in th form of array. 
      path: 'indoorEventPath'
    }).execPopulate()

    let randomisedUserFlag // this flag is use to check wether particular indoor event is present or not in randomisedUser model.
    ...
    if (randomisedUserFlag) {
      ...
      // the following 'if' logic will check users present in randomisedUser Model and ramdomisiedUsersInner varible, 
      //this checking is done bcoz if any new user has resgistered for particular event then that user will 
      //be added(updated) in particular object of randomised user)
    ...
    } else {//if particular event is not present in randomisedUser model then that event is added 
...
function randomisation(Events,eventType) {
  ...
  if(eventType == 'indoor'){
    filteredArray = Events.indoorEventPath.filter(value => {  // indoorEventPath contain array of user registered in particular event, this array is created by retriving data from eventRegeistered model.
      return value.userInEventFlag === true   // if user lost in any match then this flag will become false and user is considered out of particular event
    })
  }
